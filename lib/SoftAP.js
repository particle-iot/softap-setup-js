...
SoftAP.prototype.deviceInfo = function deviceInfo(cb) {
	...
	this.__sendCommand({ name: 'device-id' }, function response(err, dat) {
		...
		const claimed = dat.c === '1';
		const id = dat.id && dat.id.toLowerCase();
		this.__deviceID = id;
...
SoftAP.prototype.publicKey = function publicKey(cb) {
	...
	this.__sendCommand({ name: 'public-key' }, function response(err, dat) {
		checkResponse(err, dat, cb);
		const buff = new Buffer(dat.b, 'hex');
...
SoftAP.prototype.aesEncrypt = function aesEncrypt(data, kiv) {
	if (!kiv) {
		kiv = crypto.randomBytes(32);
	}
	const kivEncrypted = this.__publicKey.encrypt(kiv, 'hex');
	const cipher = crypto.createCipheriv('aes-128-cbc', kiv.slice(0, 16), kiv.slice(16, 32));
...
SoftAP.prototype.configure = function configure(opts, cb) {
	...
	if (opts.security & securityTable.enterprise) {
		...
		// eslint-disable-next-line eqeqeq
			...
			const enc = this.aesEncrypt(formatPem(opts.private_key));
...
SoftAP.prototype.securityValue = function securityValue(name) {
...
SoftAP.prototype.eapTypeValue = function eapTypeValue(name) {
